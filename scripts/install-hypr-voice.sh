...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    cp "$WHISPER_CONFIG_DIR/hypr-voice.service" "$HOME/.config/systemd/user/"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    ...
    # Copy hyprvoice.conf
    cp "$WHISPER_CONFIG_DIR/hyprvoice.conf" "$HYPRLAND_CONFIG/"
    ...
    echo -e "       1. Copy: cp $WHISPER_CONFIG_DIR/hyprvoice.conf ~/.config/hypr/"
...
echo -e "  ${BLUE}$PROJECT_ROOT/.venv/bin/python -m hypr_voice.whisper.integration.hypr_voice_type start${NC}"
