#!/bin/bash
# Quick start script for ydotool integration - Microphone mode (for voice input)
...
echo -e "${BLUE}🎤 Starting ydotool Voice-to-Type (Microphone Mode)${NC}"
echo ""
...
# Run with optimized settings: word mode, fast typing, NO WAIT - run as module
"$VENV_PATH/bin/python" -m hypr_voice.whisper.integration.wltype_integration --realtime --use-microphone --word-mode --stability-checks 0
