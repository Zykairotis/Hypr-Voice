...
# Configuration
CONFIG_FILE="$WHISPER_CONFIG_DIR/config.yaml"
...
start_server() {
    ...
    # Python files have moved to src/hypr_voice/whisper/
    # Run directly as script to avoid parent package import issues
    cd "$PROJECT_ROOT"
    # Set PYTHONPATH to include src/ for module imports
    export PYTHONPATH="$PROJECT_ROOT/src:$PYTHONPATH"
    nohup "$VENV_PATH/bin/python" "$PROJECT_ROOT/src/hypr_voice/whisper/core/hybrid_server.py" > "$LOG_FILE" 2>&1 &
    local server_pid=$!
