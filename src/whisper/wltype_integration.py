...
def check_wltype_installed():
    ...
    try:
        ...
        print("  Install with: sudo apt-get install wtype (Wayland)")
...
def type_text(text, typing_speed=0.01):
    """Type text using wtype with configurable speed (use type_text_instant for fastest)."""
    for char in text:
        subprocess.run(["wtype", char], capture_output=True)
        time.sleep(typing_speed)

def type_text_instant(text):
    """Type text instantly using wtype with 1ms delay (fastest possible)."""
    subprocess.run(["wtype", "-d", "1", text], capture_output=True)

def type_text_words(text, words_per_second=5):
    """Type text word-by-word using wtype - MUCH faster than character-by-character."""
    words = text.split()
...
class RealtimeTyper:
    ...
    def __init__(self, server_url="http://localhost:9090", device=None, typing_speed=0.02, sample_rate=16000, stability_checks=1, use_word_mode=True):
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Hybrid Whisper with wtype Auto-Typing"
    )
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Check if wtype is installed"
    )
    
    args = parser.parse_args()
    
    # Check wtype installation
    if args.check:
